...
define(['jsdom', 'fs', 'module', 'path', 'Dobject', 'SynerJ', 'cssom',
  'config', 'sox', 'jquery', 'node-native-zip'],
  ...
  var Page = (function () {
    ...
    Page.prototype.toExportHTML = function () {
      ...
      return html.replace('<script data-main="client/main" src="public/libs/require.js"></script>',
        '<script data-main="js/main" src="js/require.js"></script>')
          ...
          .replace('<link rel="stylesheet" type="text/css" href="public/css/car.css" />',
    ...
    Page.prototype.export = function (callback) {
      ...
      var css = this.toCSS();
      exp.add("js/data.js", new Buffer(data, 'utf-8'));
      exp.add("css/" + name + ".css", new Buffer(css, 'utf-8'));
      exp.add("index.html", new Buffer(html, 'utf-8'));
      exp.addFiles([
        ...
        var buf = exp.toBuffer();
      
        fs.writeFile('public/export/' + name + ".zip", buf, function (err) {
          if (err)
            console.log(err);
          if (callback)
            callback(buf);
